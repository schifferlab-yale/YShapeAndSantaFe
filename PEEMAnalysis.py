...
if __name__ == "__main__":
    ...
    for islandIndex,islands in enumerate(islandData):
        rotated=rotatePEEM.rotatePEEM(rowData,colData,islands)
        rotated=rotated.split("\n")

        #print(rotated)
        lattice=santafeAnalysis.SantaFeLattice(rotated, firstCenter=(1,-23), removeEdgeStrings=False)
        #lattice.removeSmallStrings()
        #lattice.removeStringsNotConnectingInteriors()
        strings=lattice.strings

        #print(strings[list(strings.keys())[2]].asGraph())
